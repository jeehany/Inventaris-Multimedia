                            ...
                            <?php if(auth()->guard()->check()): ?>
                                <?php if(auth()->user()->isKepala()): ?>
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" class="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-xl z-20 border border-slate-100" style="display: none;">
                                            <a href="<?php echo e(route('tools.exportPdf', request()->query())); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-emerald-600 border-b border-slate-100">
                                                📄 Laporan Aset (PDF)
                                            </a>
                                ...
                                <?php else: ?>
                                    <div class="flex gap-2 w-full md:w-auto">
                                        ...
                                        <a href="<?php echo e(route('tools.printQr', request()->query())); ?>" target="_blank" class="inline-flex items-center justify-center bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-2 px-5 rounded-lg shadow-sm transition duration-150 ease-in-out text-sm gap-2">
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex justify-center items-center space-x-2">
                                                ...
                                                <a href="<?php echo e(route('tools.printSingleQr', $tool->id)); ?>" target="_blank"
                                                ...
                                                <?php if(auth()->guard()->check()): ?>
                                                    <?php if(!auth()->user()->isKepala()): ?>
                                                        ...
                                                        <button type="button" onclick="toggleModal('modal-edit-<?php echo e($tool->id); ?>')" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition" title="Edit">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        ...
                                                        <button type="button" 
                                                                onclick="openDeleteModal('<?php echo e(route('tools.destroy', $tool->id)); ?>', 'Hapus Aset <?php echo e($tool->tool_name); ?>?', 'Anda akan menghapus aset <strong><?php echo e($tool->tool_name); ?></strong> (<span class=\'font-mono\'><?php echo e($tool->tool_code); ?></span>).<br>Data akan dipindahkan ke <strong>Trash (Sampah)</strong>.')" 
                                                    ...
                                                    <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full border border-slate-200">
                                                        ...
                                                        <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse border-t border-slate-100">
                                                            ...
                                                            <?php if(auth()->user()->isStaff()): ?>
                                                                <button type="button" onclick="document.getElementById('modal-detail-<?php echo e($tool->id); ?>').classList.add('hidden'); toggleModal('modal-edit-<?php echo e($tool->id); ?>');" class="mr-3 w-full inline-flex justify-center rounded-lg shadow-sm px-4 py-2 bg-amber-500 text-base font-medium text-white hover:bg-amber-600 focus:outline-none sm:w-auto sm:text-sm transition">
                                                                    Edit Data
                                                                </button>
                                                                <a href="<?php echo e(route('tools.printSingleQr', $tool->id)); ?>" target="_blank" class="mr-3 w-full inline-flex justify-center items-center gap-2 rounded-lg shadow-sm px-4 py-2 bg-emerald-500 text-base font-medium text-white hover:bg-emerald-600 focus:outline-none sm:w-auto sm:text-sm transition">
                                            ...
                                            <?php echo $__env->make('tools.modal_edit', ['tool' => $tool], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <div id="modal-create" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-hidden="true" role="dialog">
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
            ...
            <div class="inline-block bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:max-w-2xl sm:w-full">
                <form action="<?php echo e(route('tools.store')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="category_id" required class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-700">
                                    ...
                                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($category->id); ?>"><?php echo e($category->category_name); ?> (<?php echo e($category->code); ?>)</option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
...
<?php $component->withAttributes(['id' => 'deleteModal']); ?>
        ...
        <div class="bg-rose-50 p-4 rounded-xl border border-rose-100 text-left mt-4">
