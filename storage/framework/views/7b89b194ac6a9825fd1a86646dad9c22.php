            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200">
                <div class="p-6 md:p-8 text-slate-800">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-8">
                        ...
                        <form action="<?php echo e(route('borrowers.index')); ?>" method="GET" class="flex w-full md:w-auto relative bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-all">
                            <div class="relative w-full md:w-64 group">
                                ...
                                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari Nama / ID Anggota..." 
                            ...
                            <?php if(request('search')): ?>
                                <a href="<?php echo e(route('borrowers.index')); ?>" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors flex items-center">
                    ...
                    <div class="overflow-x-auto rounded-xl border border-slate-200 shadow-sm">
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                <?php $__empty_1 = true; $__currentLoopData = $borrowers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-6 py-4 text-center text-sm text-slate-500 font-medium">
                                            <?php echo e($borrowers->firstItem() + $index); ?>

                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex-shrink-0 h-12 w-12">
                                                <?php if($b->photo): ?>
                                                    <img class="h-12 w-12 rounded-full object-cover border-2 border-indigo-100 shadow-sm" src="<?php echo e(asset('storage/'.$b->photo)); ?>" alt="">
                                                <?php else: ?>
                                            ...
                                            <?php if(auth()->guard()->check()): ?>
                                                <?php if(!auth()->user()->isKepala()): ?>
                                                    <div class="flex justify-center items-center gap-3">
                                                        <button onclick="toggleModal('modal-edit-<?php echo e($b->id); ?>')" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-2 rounded-lg hover:bg-indigo-100 transition" title="Edit">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                        </button>
                                                        <button type="button" onclick="openDeleteModal('<?php echo e(route('borrowers.destroy', $b->id)); ?>', 'Hapus Anggota <?php echo e($b->name); ?>?', 'Yakin ingin menghapus data anggota ini? Riwayat peminjaman terkait mungkin akan terdampak.')" class="text-rose-600 hover:text-rose-900 bg-rose-50 p-2 rounded-lg hover:bg-rose-100 transition" title="Hapus">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                        </button>
                                                    </div>

                                                    
                                                    <?php echo $__env->make('borrowers.modal_edit', ['b' => $b], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                                                <?php else: ?>
                    ...
                    <div class="mt-8"><?php echo e($borrowers->links()); ?></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="modal-create" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-hidden="true" role="dialog">
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
            ...
            <div class="inline-block w-full max-w-md bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all">
                <form method="POST" action="<?php echo e(route('borrowers.store')); ?>" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    <div class="bg-white px-6 pt-6 pb-6">
                        <div class="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
                            <h3 class="text-lg font-bold text-slate-900">Tambah Anggota Baru</h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <!-- Photo -->
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Foto Profil (Opsional)</label>
                    ...
                    <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-100">
                        <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 text-sm font-semibold shadow-md transition">Simpan Anggota</button>
...
<?php endif; ?><?php /**PATH C:\laragon\www\app-inventaris\resources\views/borrowers/index.blade.php ENDPATH**/ ?>