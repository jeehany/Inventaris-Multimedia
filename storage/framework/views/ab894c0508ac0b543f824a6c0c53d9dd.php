...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <h2 class="font-bold text-xl text-slate-800 leading-tight">
            <?php echo e(__('Monitoring Status Aset Real-Time')); ?>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex items-center justify-between hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-slate-800"><?php echo e($stats['total_tools']); ?></h4>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-emerald-100 p-5 flex items-center justify-between hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-emerald-700"><?php echo e($stats['available']); ?></h4>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-blue-100 p-5 flex items-center justify-between hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-blue-700"><?php echo e($stats['borrowed']); ?></h4>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-amber-100 p-5 flex items-center justify-between hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-amber-700"><?php echo e($stats['maintenance']); ?></h4>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-rose-100 p-5 flex items-center justify-between hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-2xl font-bold text-rose-700"><?php echo e($stats['disposed']); ?></h4>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                ...
                <div class="px-6 py-5 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50/50">
                    ...
                    <form method="GET" action="<?php echo e(route('monitoring.index')); ?>" class="w-full md:w-auto flex items-center gap-2 relative">
                        <select name="status" class="w-full pl-3 pr-10 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm appearance-none bg-white">
                            ...
                            <option value="available" <?php echo e(request('status') == 'available' ? 'selected' : ''); ?>>Tersedia / Gudang</option>
                            <option value="borrowed" <?php echo e(request('status') == 'borrowed' ? 'selected' : ''); ?>>Dipinjam</option>
                            <option value="maintenance" <?php echo e(request('status') == 'maintenance' ? 'selected' : ''); ?>>Dalam Perbaikan</option>
                            <option value="disposed" <?php echo e(request('status') == 'disposed' ? 'selected' : ''); ?>>Dihapus / Hilang</option>
                        </select>
                            ...
                            <?php $__empty_1 = true; $__currentLoopData = $tools; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tool): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50/70 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="font-mono text-sm font-bold text-slate-600 bg-slate-100 px-2.5 py-1 rounded border border-slate-200"><?php echo e($tool->tool_code); ?></span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <?php if($tool->photo): ?>
                                                <img src="<?php echo e(Storage::url($tool->photo)); ?>" class="w-10 h-10 rounded-lg object-cover border border-slate-200">
                                            <?php else: ?>
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-slate-500"><?php echo e($tool->category ? $tool->category->category_name : 'Tanpa Kategori'); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold <?php echo e($toolDetails[$tool->id]['status_badge']); ?>">
                                            <?php echo e($toolDetails[$tool->id]['status_label']); ?>

                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-medium text-slate-700">
                                            <?php echo e($toolDetails[$tool->id]['pic']); ?>

                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <span class="text-sm text-slate-600">
                                            <?php echo e($toolDetails[$tool->id]['location']); ?>
                                ...
                                <div class="border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition relative overflow-hidden group">
                                    ...
                                    <div class="relative z-10">
                                        <div class="flex justify-between items-start mb-3">
                                            <span class="px-2 py-1 bg-amber-100 text-amber-800 text-[10px] font-bold uppercase rounded"><?php echo e($maintenance->status == 'in_progress' ? 'SEDANG DISERVIS' : 'PENDING'); ?></span>
                                            <span class="text-xs text-slate-400 font-medium">Beban: Mulai <?php echo e(\Carbon\Carbon::parse($maintenance->start_date)->format('d M y')); ?></span>
                                        </div>
                                        <h4 class="font-bold text-slate-800 mb-1"><?php echo e($maintenance->tool ? $maintenance->tool->tool_name : 'Unknown Tool'); ?></h4>
                                        <p class="text-sm text-slate-600 border-l-2 border-amber-300 pl-2 mb-4 bg-slate-50 py-1 italic shadow-sm"><?php echo e($maintenance->type ? $maintenance->type->name : 'Perawatan Umum'); ?></p>
                                        
                                        <div class="text-xs text-slate-500 flex flex-col gap-1">
                                            <div class="flex items-center gap-1.5">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                                <strong>Teknisi:</strong> <?php echo e($maintenance->user ? $maintenance->user->name : '-'); ?>

                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                <strong>Catatan:</strong> <?php echo e(Str::limit($maintenance->note, 40)); ?>
                                        ...
                                        <?php if($maintenance->cost > 0): ?>
                                            <div class="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-sm font-bold text-slate-700">
                                                ...
                                                <span class="text-rose-600">Rp <?php echo e(number_format($maintenance->cost, 0, ',', '.')); ?></span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\app-inventaris\resources\views/monitoring/index.blade.php ENDPATH**/ ?>