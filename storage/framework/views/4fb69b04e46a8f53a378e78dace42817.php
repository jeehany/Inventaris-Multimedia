            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-800"><?php echo e($totalMaintenance); ?></p>
                    </div>
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-800"><?php echo e($minProgress); ?></p>
                    </div>
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-800"><?php echo e($completed); ?></p>
                    </div>
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-lg font-bold text-slate-800">Rp <?php echo e(number_format($totalCost, 0, ',', '.')); ?></p>
                    </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200">
                <div class="p-6 md:p-8 text-slate-800">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-8">
                        ...
                        <form id="filterForm" action="<?php echo e(route('maintenances.index')); ?>" method="GET" class="w-full md:w-auto flex flex-col md:flex-row gap-2 items-center bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-all">
                            ...
                            <div class="w-full md:w-auto relative group">
                                ...
                                <select name="status" onchange="this.form.submit()" class="w-full md:w-40 pl-8 pr-8 py-2 border-none bg-transparent rounded-lg text-sm text-slate-600 font-medium focus:ring-0 cursor-pointer hover:text-indigo-700 transition-colors appearance-none">
                                    ...
                                    <option value="in_progress" <?php echo e(request('status') == 'in_progress' ? 'selected' : ''); ?>>Sedang Proses</option>
                            ...
                            <?php if(request('search') || request('status') || request('type_id')): ?>
                                <a href="<?php echo e(route('maintenances.index')); ?>" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Reset Filter">
                            ...
                            <?php if(auth()->user()->isHead()): ?>
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" class="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-xl z-20 border border-slate-100" style="display: none;">
                                            <a href="<?php echo e(route('maintenances.exportPdf', request()->query())); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-emerald-600">
                                                📄 Laporan Pemerliharaan (PDF)
                                            </a>
                                            <a href="<?php echo e(route('maintenances.export', request()->query())); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-green-600">
                                                📗 Laporan Pemerliharaan (Excel)
                                            </a>
                                        </div>
                                    </div>
                            <?php else: ?>
                                <a href="<?php echo e(route('maintenances.create')); ?>" class="w-full md:w-auto inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-5 rounded-lg shadow-lg hover:shadow-indigo-500/30 transition duration-150 ease-in-out text-sm gap-2">
                    ...
                    <div x-data="{ 
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                <?php $__empty_1 = true; $__currentLoopData = $maintenances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 text-center">
                                            <?php echo e($maintenances->firstItem() + $index); ?>

                                        </td>
                                        
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex flex-col">
                                                <span class="text-sm font-bold text-slate-800"><?php echo e($item->tool->tool_name ?? 'Aset Dihapus'); ?></span>
                                                <span class="text-xs text-slate-500"><?php echo e($item->tool->tool_code ?? '-'); ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col gap-1">
                                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-50 text-indigo-700 w-fit">
                                                    <?php echo e($item->type->name ?? 'Umum'); ?>

                                                </span>
                                                <span class="text-sm text-slate-600 truncate max-w-xs" title="<?php echo e($item->note); ?>">
                                                    <?php echo e(Str::limit($item->note, 30)); ?>

                                                </span>
                                                <span class="text-xs text-slate-400">Pelapor: <?php echo e($item->user->name ?? '-'); ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-700">
                                            Rp <?php echo e(number_format($item->cost, 0, ',', '.')); ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            <?php else: ?>
                                                ...
                                                <div class="text-[10px] text-slate-400 mt-1">
                                                    <?php echo e($item->end_date ? \Carbon\Carbon::parse($item->end_date)->format('d/m/Y') : ''); ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            <div class="flex justify-center items-center space-x-3">
                                                ...
                                                <button 
                                                    @click="openModal({
                                                        tool_name: '<?php echo e(addslashes($item->tool->tool_name ?? '-')); ?>',
                                                        tool_code: '<?php echo e($item->tool->tool_code ?? '-'); ?>',
                                                        reporter: '<?php echo e(addslashes($item->user->name ?? '-')); ?>',
                                                        type: '<?php echo e(addslashes($item->type->name ?? '-')); ?>',
                                                        start_date: '<?php echo e(\Carbon\Carbon::parse($item->start_date)->translatedFormat('d F Y')); ?>',
                                                        end_date: '<?php echo e($item->end_date ? \Carbon\Carbon::parse($item->end_date)->translatedFormat('d F Y') : '-'); ?>',
                                                        cost: '<?php echo e(number_format($item->cost, 0, ',', '.')); ?>',
                                                        status: '<?php echo e($item->status); ?>',
                                                        note: `<?php echo e(addslashes(preg_replace( "/\r|\n/", " ", $item->note ?? ''))); ?>`,
                                                        action_taken: `<?php echo e(addslashes(preg_replace( "/\r|\n/", " ", $item->action_taken ?? '-'))); ?>`
                                                    })"
                                                ...
                                                <?php if(auth()->guard()->check()): ?>
                                                    <?php if(!auth()->user()->isHead()): ?>
                                                        ...
                                                        <a href="<?php echo e(route('maintenances.edit', $item->id)); ?>" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition" title="Edit / Selesaikan">
                                                        ...
                                                        <button type="button" onclick="openDeleteModal('<?php echo e(route('maintenances.destroy', $item->id)); ?>', 'Hapus Data Perbaikan?', 'Yakin ingin menghapus data ini? Status alat akan dikembalikan ke kondisi sebelumnya.')" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition" title="Hapus">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="mt-8">
                        <?php echo e($maintenances->withQueryString()->links()); ?>
...
<?php endif; ?><?php /**PATH C:\laragon\www\app-inventaris\resources\views/maintenances/index.blade.php ENDPATH**/ ?>