...
<?php $component->withAttributes([]); ?>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                ...
                                <div class="bg-slate-50 p-5 rounded-xl border border-slate-200">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                                        <div>
                                            ...
                                            <input type="date" name="borrow_date" id="borrow_date" value="<?php echo e(old('borrow_date', date('Y-m-d'))); ?>" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" required>
                                        </div>
                                        <div>
                                            <label for="return_date" class="block text-xs font-semibold text-slate-700 mb-1">Rencana Kembali</label>
                                            <input type="date" name="return_date" id="return_date" value="<?php echo e(old('return_date')); ?>" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" required>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="lg:col-span-8 flex flex-col h-full bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                                ...
                                <div class="bg-slate-100 border-t border-slate-200 p-5 flex items-center justify-end gap-3 mt-auto">
                                    <a href="<?php echo e(route('borrowings.index')); ?>" class="text-slate-500 hover:text-slate-800 font-semibold px-4 py-2 transition text-sm">Kembali</a>
    ...
    <script>
        ...
        async function submitBorrowerAjax(e) {
            ...
            formData.append('_token', '<?php echo e(csrf_token()); ?>'); // Explicit CSRF, though meta is also fine

            try {
                const response = await fetch('<?php echo e(route('borrowers.storeAjax')); ?>', {
                    method: 'POST',
                    headers: {
