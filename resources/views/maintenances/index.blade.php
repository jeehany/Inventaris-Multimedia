                        ...
                        <table class="min-w-full table-auto border-collapse border border-gray-200 text-sm">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    {{-- UPDATE: KOLOM JENIS MAINTENANCE --}}
                            ...
                            <tbody>
                                @forelse($maintenances as $key => $item)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        {{-- UPDATE: TAMPILKAN DATA JENIS --}}
                                        <td class="border px-4 py-2">
                                            @if($item->type)
                                                <span class="bg-indigo-100 text-indigo-800 text-xs font-semibold px-2.5 py-0.5 rounded border border-indigo-300">
                                                    {{ $item->type->name }}
                                                </span>
                                            @else
                                        ...
                                        <td class="border px-4 py-2 text-center">
                                            <div class="flex justify-center gap-2">
                                                ...
                                                {{-- Tombol Hapus --}}
                                                <form action="{{ route('maintenances.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini? Jika status masih perbaikan, alat akan otomatis menjadi Available.');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="bg-red-100 text-red-600 p-1.5 rounded hover:bg-red-200">
                                                        🗑️
                                                    </button>
                                                </form>
                                            </div>
