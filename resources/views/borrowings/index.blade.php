        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200">
                <div class="p-6 md:p-8 text-slate-800">
                    ...
                    {{-- Quick Return Scanner Bar & Admin Add Transaction --}}
                    @auth
                        @if(in_array(auth()->user()->role, ['admin', 'staff']))
                            <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                                
                                {{-- Quick Return Scanner Bar (KIRI) --}}
                                ...
                                {{-- Tombol Peminjaman Baru (KANAN) --}}
                                <div class="w-full md:w-auto h-full flex items-start justify-end mt-1 md:mt-0">
                                    <a href="{{ route('borrowings.create') }}" class="w-full md:w-auto shrink-0 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg hover:shadow-indigo-500/30 transition duration-200 ease-in-out transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            <div class="flex justify-center items-center space-x-2">
                                                ...
                                                @auth
                                                    @if(auth()->user()->isKepala())
                                                        @if($borrowing->borrowing_status == 'pending_head')
                                                            {{-- Approve Kepala --}}
                                                            ...
                                                            {{-- Reject Kepala --}}
                                                            <button onclick="toggleModal('modal-reject-{{ $borrowing->id }}')" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition" title="Tolak Peminjaman">
                                                               <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                            </button>
                                                        @endif
                                                    @else
                                            ...
                                            {{-- MODAL REJECT (Kepala Only) --}}
                                            @auth
                                                @if(auth()->user()->isKepala() && $borrowing->borrowing_status == 'pending_head')
                                                    <div id="modal-reject-{{ $borrowing->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-hidden="true">
                                                        <div class="flex items-center justify-center min-h-screen px-4 text-center">
                                                            <div class="fixed inset-0 bg-slate-900 bg-opacity-75 transition-opacity" onclick="toggleModal('modal-reject-{{ $borrowing->id }}')"></div>
                                                            <div class="inline-block bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full border border-slate-200">
                                                                <form action="{{ route('borrowings.reject', $borrowing->id) }}" method="POST">
                                                                    ...
                                                                    <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3">
                                                                        ...
                                                                        <button type="button" onclick="toggleModal('modal-reject-{{ $borrowing->id }}')" class="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 text-sm font-medium transition">Batal</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
