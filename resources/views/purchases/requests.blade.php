                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            <div class="flex justify-center items-center space-x-2">
                                                ...
                                                @elseif(auth()->user()->role == 'bendahara' && $purchase->status == 'approved_head')
                                                    {{-- Pencairan Bendahara --}}
                                                    <button onclick="openEvidenceModal({{ $purchase->id }}, {{ $purchase->total_amount }})" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition" title="Proses Pencairan & Bukti Belanja">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                    </button>
                                                    {{-- Reject Bendahara --}}
                                                    <button type="button" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition inline" title="Tolak Pencairan" onclick="rejectPurchase({{ $purchase->id }}, 'Alasan Penolakan (Misal: Tidak Ada Anggaran):')">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                                    </button>
                                                    <form id="reject-form-{{ $purchase->id }}" action="{{ route('purchases.reject', $purchase->id) }}" method="POST" class="hidden">
                                                        @csrf @method('PATCH')
                                                ...
                                                @elseif(auth()->user()->role == 'staff' && str_starts_with($purchase->status, 'pending'))
