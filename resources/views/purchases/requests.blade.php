                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-xs text-slate-500 mt-1 flex flex-col gap-0.5">
                                                <span><span class="font-medium text-slate-700">Vendor:</span> {{ $purchase->vendor ? $purchase->vendor->name : '-' }}</span>
                                                <span><span class="font-medium text-slate-700">Total Item:</span> {{ $purchase->items ? $purchase->items->sum('quantity') : 0 }} unit</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            @if($purchase->status == 'pending_head' || $purchase->status == 'pending')
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 border border-amber-200">
                                                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                    Menunggu Kepala
                                                </span>
                                            @elseif($purchase->status == 'approved_head')
                                            ...
                                            @elseif(str_contains($purchase->status, 'rejected'))
                                                <div class="flex flex-col items-center gap-1">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                            <div class="flex justify-center items-center space-x-2">
                                                ...
                                                @if(auth()->user()->role == 'kepala' && str_starts_with($purchase->status, 'pending'))
                                                    {{-- Approve Kepala --}}
                                                    <form action="{{ route('purchases.approve', $purchase->id) }}" method="POST" class="inline">
                                                        @csrf @method('PATCH')
                                                    ...
                                                    {{-- Reject Kepala --}}
                                                    <button type="button" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition inline" title="Tolak Pengajuan" onclick="rejectPurchase({{ $purchase->id }}, 'Alasan Penolakan Kepala:')">
                                                ...
                                                @elseif(auth()->user()->role == 'bendahara')
                                                    @if($purchase->status == 'approved_head')
                                                        {{-- UPLOAD BUKTI (BENDAHARA) --}}
                                                        <button type="button" onclick="openEvidenceModal({{ $purchase->id }}, {{ $purchase->total_amount }})" class="text-emerald-600 hover:text-emerald-900 bg-emerald-50 hover:bg-emerald-100 p-2 rounded-lg transition inline-block" title="Upload Kwitansi & Cairkan Anggaran">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                        </button>
                                                    @endif
                                                @elseif(auth()->user()->role == 'staff')
                                                    @if(str_starts_with($purchase->status, 'pending'))
                                                    ...
                                                    @elseif($purchase->status == 'approved_head')
                                                        {{-- CETAK SURAT QR (STAFF) --}}
                                                        <a href="{{ route('purchases.surat_perintah', $purchase->id) }}" target="_blank" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition inline-block" title="Cetak Surat Perintah (QR)">
