                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="font-bold text-slate-800">{{ $p->purchase_code }}</div>
                                        <div class="text-xs text-slate-500 mt-0.5">
                                            {{ \Carbon\Carbon::parse($p->date)->format('d M Y') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-indigo-700">{{ $p->tool_name }}</div>
                                        <div class="text-xs text-slate-500 mt-0.5">{{ $p->category->category_name ?? '-' }}</div>
                                        @if($p->specification)
                                            <div class="mt-1 text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded inline-block">
                                                {{ $p->specification }}
                                            </div>
                                        @endif
                                        <div class="mt-1 font-medium text-slate-600 text-xs">Qty: {{ $p->quantity }} Unit</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-600">
                                        {{ $p->vendor->name }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="text-slate-400 text-xs">Est. Unit: Rp {{ number_format($p->unit_price, 0, ',', '.') }}</div>
                                        <div class="font-bold text-slate-800">Rp {{ number_format($p->subtotal, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @if(Auth::user()->role == 'head')
                                            ...
                                            <span class="text-slate-400 text-xs italic bg-slate-100 px-2 py-1 rounded">Read-only</span>
                                        @else
                                            {{-- BUTTON UNTUK ADMIN --}}
                                            <button 
