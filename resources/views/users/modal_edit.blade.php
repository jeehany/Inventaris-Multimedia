{{-- MODAL EDIT USER --}}
<div id="modal-edit-{{ $user->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-hidden="true" role="dialog">
    <div class="flex items-center justify-center min-h-screen px-4 text-center">
        <!-- Backdrop -->
        <div class="fixed inset-0 bg-slate-900 bg-opacity-75 transition-opacity backdrop-blur-sm" onclick="toggleModal('modal-edit-{{ $user->id }}')"></div>
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                <div class="bg-white px-6 pt-6 pb-6">
                    <div class="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
                        ...
                        <button type="button" onclick="toggleModal('modal-edit-{{ $user->id }}')" class="text-slate-400 hover:text-rose-500 transition-colors">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="role" required class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-700">
                                <option value="admin" {{ $user->role == 'admin' ? 'selected' : '' }}>Admin Operasional</option>
                                <option value="head" {{ $user->role == 'head' ? 'selected' : '' }}>Kepala (Verifikator)</option>
                            </select>
                        </div>
                ...
                <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-100">
                    ...
                    <button type="button" onclick="toggleModal('modal-edit-{{ $user->id }}')" class="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 text-sm font-semibold shadow-sm transition">Batal</button>
                </div>
            </form>
        </div>
    </div>
