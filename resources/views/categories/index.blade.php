            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200">
                <div class="p-6 md:p-8 text-slate-800">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-8">
                        {{-- Form Search (MODERN TOOLBAR) --}}
                        <form action="{{ route('categories.index') }}" method="GET" class="w-full md:w-auto flex flex-col md:flex-row gap-2 items-center bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-all">
                            ...
                            {{-- Reset Button --}}
                            @if(request('search'))
                                <a href="{{ route('categories.index') }}" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Reset Filter">
                                            ...
                                            @auth
                                                @if(!auth()->user()->isKepala())
                                                    <div class="flex justify-center items-center space-x-2">
                                                        {{-- Edit --}}
                                                        <button onclick="toggleModal('modal-edit-{{ $cat->id }}')" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition" title="Edit">
