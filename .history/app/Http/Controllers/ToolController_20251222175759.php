...
class ToolController extends Controller
{
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $hasHistory = method_exists($tool, 'borrowingItems') && $tool->borrowingItems()->exists();

        // Jika ada riwayat dan tidak ada parameter force, tolak penghapusan
        if ($hasHistory && !$request->input('force')) {
            return redirect()->route('tools.index')->with('error', 'Alat memiliki riwayat peminjaman. Gunakan opsi hapus paksa untuk menghapus beserta riwayat.');
        }

        DB::transaction(function() use ($tool, $hasHistory, $request) {
            if ($hasHistory && $request->input('force')) {
                // Hapus riwayat peminjaman (jika Anda mau menyimpan history, jangan lakukan ini)
                $tool->borrowingItems()->delete();
            }
            $tool->delete();
        });
