    ...
    public function process(Request $request, $id)
{
    ...
    // Update status (sesuaikan dengan alur Anda, misal jadi 'verifying' atau 'completed')
    $purchase->status = 'verifying'; 

    // 4. LOGIKA SIMPAN GAMBAR (Ini yang kemarin mungkin kurang)
    if ($request->hasFile('proof_photo')) {
        // Hapus foto lama jika ada (agar server tidak penuh sampah)
        if ($purchase->proof_photo) {
            Storage::disk('public')->delete($purchase->proof_photo);
        }

        // Simpan foto baru ke folder "evidence"
        $path = $request->file('proof_photo')->store('evidence', 'public');
        
        // Masukkan path ke database
        $purchase->proof_photo = $path;
    }

    $purchase->save();

    return redirect()->back()->with('success', 'Transaksi berhasil diproses & Nota tersimpan!');
}
}