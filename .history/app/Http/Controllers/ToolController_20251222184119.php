...
class ToolController extends Controller
{
    ...
    /**
     * Remove the specified tool from storage (hard delete).
     * Also delete borrowing_items related to the tool to avoid FK issues.
     */
    public function destroy($id)
    {
        $tool = Tool::findOrFail($id);

        // Hapus riwayat peminjaman terkait lalu hapus alat
        \DB::transaction(function() use ($tool) {
            if (method_exists($tool, 'borrowingItems')) {
                $tool->borrowingItems()->delete();
            }
            $tool->delete();
        });

        return redirect()->route('tools.index')->with('success', 'Alat dan riwayat peminjaman terkait berhasil dihapus.');
    }
}