            ...
            <tr>
                ...
                <td class="text-center">
                    @php
                        ...
                        // Gunakan actual_return_date (atau return_date) jika ada
                        $actual = $b->actual_return_date ? \Carbon\Carbon::parse($b->actual_return_date)->startOfDay() : \Carbon\Carbon::parse($b->return_date)->startOfDay();
                        $diff = $actual->diffInDays($planned, false); // Positif = Telat, Negatif = Cepat
                    @endphp

                    @if($b->borrowing_status == 'returned' || $b->return_date)
                        @if($diff > 0)
