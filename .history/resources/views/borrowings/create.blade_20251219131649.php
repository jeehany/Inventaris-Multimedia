                    ...
                    <form action="{{ route('borrowings.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="borrower_id" id="borrower_id" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                ...
                                @foreach($borrowers as $borrower)
                                    <option value="{{ $borrower->id }}">{{ $borrower->name }} (ID: {{ $borrower->code }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="borrow_date" id="borrow_date" value="{{ date('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>

                            {{-- RENCANA KEMBALI --}}
                            <div>
                                <label for="planned_return_date" class="block text-sm font-medium text-gray-700 mb-1">Rencana Kembali</label>
                                <input type="date" name="planned_return_date" id="planned_return_date" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Contoh: Keperluan acara seminar..."></textarea>
                        </div>
