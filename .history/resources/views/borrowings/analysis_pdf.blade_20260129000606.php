            ...
            <tr>
                ...
                <td class="text-center">
                    @php
                        ...
                        // Gunakan actual_return_date jika ada, kalau tidak gunakan return_date
                    ...
                    @if($b->borrowing_status == 'returned' && ($b->actual_return_date || $b->return_date))
                        ...
                        @endif
                    @else
                        {{-- ACTIVE / BELUM DIKEMBALIKAN --}}
                        @php
                            $today = now()->startOfDay();
                            $diffToday = $today->diffInDays($planned, false); // Positif = Telat, Negatif = Masih Waktu
                        @endphp
                        @if($diffToday > 0)
                            {{-- Sudah lewat dari planned return date tapi belum dikembalikan = TELAT --}}
