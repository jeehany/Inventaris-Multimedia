        ...
        <div class="inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-md sm:w-full z-10">
            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                    ...
                    <div class="mt-3">
                        <label for="code-{{ $category->id }}" class="block text-sm font-medium text-gray-700 mb-1">Kode Kategori (prefix)</label>
                        <input type="text"
                               id="code-{{ $category->id }}"
                               name="code"
                               ...
                               maxlength="10"
                               class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                               placeholder="Contoh: LPT (opsional)">
                    </div>

                    <div class="mt-3">
                        <label for="desc-{{ $category->id }}" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                        <textarea id="desc-{{ $category->id }}" name="description" rows="2" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $category->description) }}</textarea>
                    </div>
                </div>
