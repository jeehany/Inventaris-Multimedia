                                ...
                                @forelse ($tools as $index => $tool)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-4 py-3 text-center flex justify-center gap-2">
                                            ...
                                            {{-- Tombol Hapus --}}
                                            <form action="{{ route('tools.destroy', $tool->id) }}" method="POST" onsubmit="return confirmDelete(this, {{ $tool->id }});">
                                                @csrf @method('DELETE')
                                                <input type="hidden" name="force" id="force-{{ $tool->id }}" value="0">
                                                <button type="submit" data-has-history="{{ $tool->borrowingItems()->exists() ? '1' : '0' }}" class="text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1 rounded transition">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
